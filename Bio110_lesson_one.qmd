...
The rest of this tutorial will teach you how to work with data in R. It is interactive, so you'll be able to write your own code in code chunks like the one above throughout the tutorial. The code chunks also must be run in order, so if you get an error message, check and make sure you didn't forget to run a previous code chunk.
...
You can change a variable's property with the assignment arrow, in the following way: `yourDataName$variableName <- as.factor(yourDataName$variableName)`.

For example, to change the property of the `Condition` variable, you can run: `data$Condition <- as.factor(data$Condition)`.
...
In this tutorial, you won't need to worry about installing or loading packages, because they are pre-loaded for you. 
