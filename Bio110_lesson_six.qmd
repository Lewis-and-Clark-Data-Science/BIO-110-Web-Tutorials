...
We'll continue to use ggplot to make a bar graph, which was introduced in [lesson 4](Bio110_lesson_four.qmd). 
...
Hint: You learned about it in [lesson two](Bio110_lesson_two.qmd).
...
If you weren't sure which function to choose or what these functions do, review [lesson 2](Bio110_lesson_two.qmd#sec-groupby-sum).
...
First, specify your dataset, called `bargraphdata`. 
Remember to use the pipe to string the functions together. 
...
Now we can graph! We'll use the same structure we did to create a histogram, boxplot and scatterplot, but we'll use the geometry `geom_col()` to make our graph. Chain it onto the main `ggplot()` function using a `+` and use `tmtmean` as your data. 
...
Now let's learn how to add color to the graph. We can color the bars by treatment, so that each treatment is a different color. Simply type `fill=treatment` into the `aes()` command, after the x and y variables. Use a comma to separate the fill color from the x and y variables. 
...
Note that we want the minimum error bar value to be the 'mean of var1 - sd of var1' and the maximum error bar value is the 'mean of var1 + sd of var1'. 
...
Try adding this command to your bar graph. 
...
Give it a try! It's not quite as simple as changing the `facet_wrap()` to `treatment`... there's a couple other edits you have to make. Mess around a bit and see if you can figure it out. Feel free to then use the hints to help you figure out what else you need to edit in your code. 
...
You may notice that the legend looks a little odd - it is continuous and doesn't show 3 separate values. This is because R thinks the `year` data is numerical, but it should be categorical (factor). 

Try changing the year data to factor data and then rerunning the code to fix the legend. If you need a refresher on how to do that, check out [lesson 5](Bio110_lesson_five.qmd#sec-var-type). 
