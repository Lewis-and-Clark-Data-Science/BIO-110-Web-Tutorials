...
If you are unsure whether to use mean or median to describe the center of data values, a **histogram** can be very helpful. In [lesson 4](Bio110_lesson_four.qmd), you will learn how to create a histogram but for now it is worth noting that a histogram can help you see how data is distributed. When a histogram's peak is not near the center, and/or its two tails are quite asymetrical, it's best to use the *median*, not the *mean*, to summarize the central tendency.
...
In [lesson 1](Bio110_lesson_one.qmd#sec-our-data), we used a dataframe called `data` with the variables `RQ`, `Condition`, `heart_rate`, `Individual`. 
